...
{
    ESP_LOGI("\n\n\n\n1", "Available heap size is %d\n", esp_get_free_heap_size());
    ESP_LOGI("1", "Minimum free heap ever available is %d\n\n\n\n", esp_get_minimum_free_heap_size());
    int res = 0;
    ...
    memcpy(master_meta_info.device_secret, DEVICE_SECRET, strlen(DEVICE_SECRET));   //0.63k

    ESP_LOGI("\n\n\n\n2", "Available heap size is %d\n", esp_get_free_heap_size());
    ESP_LOGI("2", "Minimum free heap ever available is %d\n\n\n\n", esp_get_minimum_free_heap_size());
    /* Register Callback */
    ...
    IOT_RegisterCallback(ITE_EVENT_NOTIFY, user_event_notify_handler);          //0

    ESP_LOGI("\n\n\n\n3", "Available heap size is %d\n", esp_get_free_heap_size());
    ESP_LOGI("3", "Minimum free heap ever available is %d\n\n\n\n", esp_get_minimum_free_heap_size());
    ...
    post_reply_need = 1;
    IOT_Ioctl(IOTX_IOCTL_RECV_EVENT_REPLY, (void *)&post_reply_need);       //0

    ESP_LOGI("\n\n\n\n4", "Available heap size is %d\n", esp_get_free_heap_size());
    ESP_LOGI("4", "Minimum free heap ever available is %d\n\n\n\n", esp_get_minimum_free_heap_size());
    /* Create Master Device Resources */
    g_user_example_ctx.master_devid = IOT_Linkkit_Open(IOTX_LINKKIT_DEV_TYPE_MASTER, &master_meta_info);//2.1k
    if (g_user_example_ctx.master_devid < 0) {
        EXAMPLE_TRACE("IOT_Linkkit_Open Failed\n");
        return -1;
    }

    ESP_LOGI("\n\n\n\n5", "Available heap size is %d\n", esp_get_free_heap_size());
    ESP_LOGI("5", "Minimum free heap ever available is %d\n\n\n\n", esp_get_minimum_free_heap_size());
    /* Start Connect Aliyun Server */
    res = IOT_Linkkit_Connect(g_user_example_ctx.master_devid);     //9.5k 峰值32.8k
    if (res < 0) {
    ...
    ESP_LOGI("\n\n\n\n6", "Available heap size is %d\n", esp_get_free_heap_size());
    ESP_LOGI("6", "Minimum free heap ever available is %d\n\n\n\n", esp_get_minimum_free_heap_size());
    while (1) {
